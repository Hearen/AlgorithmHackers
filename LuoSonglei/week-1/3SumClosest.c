...
//Time Limit Exceeded
//trying to traverse in three loops;
...
//Time Limit Exceeded
//try to traverse and jump out ealier by sorting;
int threeSumClosest1 (int *nums, int numsSize, int target)
{
    ...
    {
        int j = i;
        //this storing process is critical;
        v = nums[i];
        ...
        for (int j = i + 1; j < numsSize - 1; j++)
            for (int k = j + 1; k < numsSize; k++)
            {
                ...
                if (abs (tmp - target) < abs (closestSum - target))
                    closestSum = tmp;
                //stop to move any further - since it will be bigger;
