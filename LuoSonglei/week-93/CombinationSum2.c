...
void helper(int* nums, int size, int start, int target, int* stack, int top, int*** arr, int** colSizes, int* returnSize)
{
    ...
    {
        if(i==start+1 || nums[i]!=nums[i-1]) //only different candidates can be allowed in each traversing level for further traversal;
        {
            stack[top+1] = nums[i];
            helper(nums, size, i, target-nums[i], stack, top+1, arr, colSizes, returnSize);
        }
    }
}
...
int** combinationSum2(int* nums, int size, int target, int** colSizes, int* returnSize)
{
    ...
    *returnSize = 0;
    ...
    {
        if(!i || nums[i]!=nums[i-1]) //only different candidates can be allowed in each traversing level for further traversal;
        {
            stack[top+1] = nums[i];
            helper(nums, size, i, target-nums[i], stack, top+1, &arr, colSizes, returnSize);
        }
